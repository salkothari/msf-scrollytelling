...
const NODES = [
  ...
  {type:'action',label:'If available, test respiratory/stool samples, and test use urine for eligible HIV-positive children.',sub:'Positive result → treat immediately.',via:'YES — high risk or persistent symptoms',viaColor:'#ee0202',badge:{text:'Positive test → treat immediately',color:'#44bba4'}},
...
(function () {
  ...
  function loadSvg(key) {
    return fetch('algo-' + key + '-tagged.svg?v=7')
      .then(function (r) { return r.text(); })
      .then(function (txt) {
...
// ── HWK: scrolls naturally as step 8 — no pinning. The question lives
//    in the flow-right column and flows past with the rest of the steps. ──
